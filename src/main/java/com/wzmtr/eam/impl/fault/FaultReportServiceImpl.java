...
public class FaultReportServiceImpl implements FaultReportService {
    ...
    private void _buildRes(List<FaultReportResDTO> records) {
        Set<String> positionCodes = __StreamUtil.mapToSet(records, FaultReportResDTO::getPositionCode);
        List<RegionResDTO> regionResDTOS = regionMapper.selectByNodeCodes(positionCodes);
        Map<String, RegionResDTO> regionMap = __StreamUtil.toMap(regionResDTOS, RegionResDTO::getNodeCode);
        records.forEach(a -> {
