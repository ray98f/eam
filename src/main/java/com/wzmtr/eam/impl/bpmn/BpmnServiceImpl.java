...
public class BpmnServiceImpl implements BpmnService {
    ...
    public String nextTaskKey(String procId) {
        String authorization = httpServletRequest.getHeader("Authorization-Flow");
        String param = FastFlowPathUrl.NEXT_TASK_KEY + procId;
        log.info("流程引擎nextTaskKey调用入参：[{}]", param);
        JSONObject jsonObject = JSONObject.parseObject(HttpUtil.doGet(param, authorization));
